...
/// </summary>
public partial class KomgaViewModel : BaseViewModel
{
    ...
    private async Task DownloadBookAsync(KomgaBook? book)
    {
        ...
        try
        {
            ...
            var comic = await _libraryService.DownloadFromKomgaAsync(book, progress);
            
            await Shell.Current.DisplayAlertAsync(
                "Download Complete",
                $"'{book.Name}' has been downloaded and added to your library.",
                "OK");
        }
        catch (Exception ex)
        {
            await Shell.Current.DisplayAlertAsync(
                "Download Failed",
                $"Failed to download '{book.Name}': {ex.Message}",
                "OK");
        }
        finally
        {
